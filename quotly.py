...
@ilhammansiz_cmd(pattern="(quotly|qbot) ?(.*)")
async def _(event):
    ...
    async with petercordpanda_bot.conversation(chat) as conv:
        try:
            ...
            er = await petercordpanda_bot.forward_messages(chat, reply_message)
        ...
        if response.text.startswith("Hi!"):
            ...
            await ac.delete()
            await petercordpanda_bot.send_message(event.chat_id, response.message)


# Oringinal Source from Nicegrill: https://github.com/erenmetesar/NiceGrill/
# Ported to petercordpanda
